			...
			struct buffer_head *bh, int create)
{
  ...
  block_t *idata = i_data(inode);  // All the zones(extents) in the inode.
  block_t cur_ext;                 // The current extent we are at.
  int ext_idx = 0;                 // The index of the current extent.
  int ext_cnt = 0;                 // The number of blocks represented in the inode.
  int ext_pos = 0;                 // The block address/position of the extent.
  int blk_cnt = 0;                 // The number of blocks we have seen so far.
  block_t blk_pos = 0;             // The block address of the block to map.
  ...
  while(1)
    {
      ...
      if(blk_cnt == block + 1)
	{
	  printk("blk_cnt == block + 1, %i == %i + 1, break\n", blk_cnt, (int)block);
	  break;
	}
      ...
      if(ext_idx == 9)
	{
      ...
      map_bh(bh, inode->i_sb, block_to_cpu(nr));
      mark_inode_dirty(inode);
      mark_buffer_dirty_inode(bh, inode);
      ext_idx++;
